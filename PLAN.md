...
> 基础版已在 JS 端落地（`src/lib/structure.ts`）：流程图 / 柱状图 / 表格的识别 + 美化；
> 美化入口：选中笔画 → 美化笔画（先按空间聚类分组识别，再对图元做结构级对齐/吸附）。
...
- [x] 新增 `src/lib/structure.ts`：纯规则 `detectStructure` + `beautifyStructure`
  - 流程图：rect/roundrect/diamond 节点 + arrow 连线（几何推断/attach 关系），美化后箭头端点吸附到节点边缘并写入 attach
  - 柱状图：≥3 个底部对齐、等宽的矩形，美化后统一底部与宽度
  - 表格：≥2×2 行列对齐网格（含 1D 聚类），美化后吸附为等宽等高的规整网格
  - `collectCandidates`：只对“本次美化新图元”附近的图元做结构判断，避免旧图形干扰
- [x] `beautifySelected` 重构：选中笔画先按空间聚类（`clusterStrokeGroups`，相连笔画合并识别、分离笔画各自成组），
  再逐组美化，最后做结构识别与美化；AIPanel 展示识别结果（流程图/柱状图/表格 + 置信度）
- [x] 修复多笔画美化把分离图形拼接成一个整体的问题（柱状图/流程图此前会被识别成单个形状）
- [x] 手写数字尺寸路由修复：按屏幕像素（世界尺寸×缩放）判断小书写区域，缩放画布后数字仍走数字路径
  （此前缩小画布后数字被当成大区域形状拒绝，真机 logcat 验证）
- [x] 小书写区域路由：非数字小笔画只有“明显闭环的形状”才转（正圆 0/o 保护），
  开口笔画（手写字母/汉字笔画）保持笔画，避免文字被误转形状；柱状图识别支持细竖线柱子（美化时转成等宽矩形柱）
- [x] e2e：AI 后端地址强制指向本地 e2e 服务（`LOCAL_DATA_URL`），避免平板在线时请求转发到旧模型导致结果不稳定；
  新增“手绘 3 根柱状图 → 识别为柱状图并统一底部/宽度”用例
- [x] 测试：`structure.test.ts` 8 个用例（流程图/柱状图/表格检测与美化、竖线柱子、候选过滤、笔画聚类）；e2e 41 项全过；单测 60 项全过
...
- [x] 函数曲线识别修复：CNN 没有 linear/quadratic 类且纯算法排在形状之后 → 抛物线被强制映射 triangle/hexagon；修复：纯算法函数检测提前到形状之前、`classify_shape` 强函数拟合（R²>0.88/0.92）优先返回函数、normal equations 数值稳定性（先居中，世界坐标远离 0 时病态）、离群点剔除（≤3%，笔尖误触/起点跳变，实测 R² 0.86→1.0）、native `JsDetectedShape` 补 `funcParams`；真机 365 点抛物线 → quadratic conf=0.9998
- [x] 工具栏数学工具：第一档新增三角形 / 五边形 / 七边形 / 八边形 / 星形；第二档“数学”子菜单（角度标记 / 坐标系 / 抛物线，含渲染、命中测试、SVG 导出）；坐标系支持独立调节 X/Y 轴与原点（5 个轴端/原点手柄，`params` 存储）并带刻度（nice-step 自动刻度）；e2e 38 项全过
...
- [x] 两头尖菱形识别修复：CNN 将极端长宽比菱形误判为 triangle/arrow/line（1:3 竖菱形 → triangle 0.973、3:1 横菱形 → arrow 0.999）；新增 `diamond_likeness` 几何拟合兜底——CNN 判 triangle/arrow/line 且菱形拟合度 >0.65 时改判 diamond，真实三角形/箭头不受影响；JS 兜底菱形宽高比放宽到 0.25~4.0（见本次提交）
