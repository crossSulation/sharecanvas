    ...
    <header className="z-30 flex h-14 shrink-0 items-center gap-3 border-b border-zinc-200 bg-white/90 px-4 backdrop-blur">
      ...
      <div className="ml-1 flex items-center gap-1">
        ...
        <button
          ...
          disabled={!hasContent}
          title="清除屏幕（当前视野内）"
          className="flex h-8 w-8 items-center justify-center rounded-md text-zinc-600 transition-colors hover:bg-zinc-100 hover:text-red-500 disabled:opacity-30"
        >
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      ...
      {penDetected && (
        <div
          ...
          className="hidden items-center gap-1.5 rounded-full border border-zinc-200 bg-zinc-50 px-3 py-1 text-xs text-zinc-500 md:flex"
          title="检测到数位板/压感笔，笔迹粗细随压感变化"
        >
      ...
      <div
        ...
        className={`hidden items-center gap-1.5 rounded-full border px-3 py-1 text-xs md:flex ${
          wsStatus === 'online'
      ...
      <button
        ...
        className="flex items-center gap-1.5 rounded-lg bg-zinc-900 px-3.5 py-1.5 text-xs font-semibold text-white transition-colors hover:bg-zinc-700"
      >
